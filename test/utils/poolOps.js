...
/**
 *  Swap given ETH for given token type and deposit tokens into Paycer pool
 *
 * @param {object} pool Paycer pool instance where we want to deposit tokens
 ...
 * @param {object} depositor User who will pay ETH and also deposit in Paycer pool
 * @returns {Promise<BigNumber>} Promise of collateral amount which was deposited in Paycer pool
 */
...
/**
 * Simulates profit in a Paycer Pool
 * As of now simulating profits behaves exactly like harvestYearn
 *
 * @param {object} strategy - strategy object
 */ 
async function harvestPaycer(strategy) {
...
async function rebalanceStrategy(strategy) {
  ...
  try {
    ...
    if (strategy.type === 'earnPaycer') {
      await harvestPaycer(strategy)
    }

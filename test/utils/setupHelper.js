...
// Contract names
const IPaycerPool = 'IPaycerPoolTest'
const CToken = 'CToken'
...
/**
 * Create and configure Paycer Maker Strategy. Also update test class object with required data.
 *
 * @param {object} poolAddress pool address
 * @param {object} strategyName Strategy name
 ...
 * @returns {object} Strategy instance
 */
async function createPaycerMakerStrategy(poolAddress, strategyName, options) {
...
async function createStrategy(strategy, poolAddress, options = {}) {
  ...
  } else if (strategyType === StrategyType.PAYCER_MAKER || strategyType === StrategyType.EARN_PAYCER_MAKER) {
    instance = await createPaycerMakerStrategy(poolAddress, strategy.name, options)
  ...
  const strategyTokenName =
    strategyType === StrategyType.PAYCER_MAKER
      ? IPaycerPool
 ...
 * @property {object} [vPool] - Optional. Paycer pool instance
 * @property {string} feeCollector - Fee collector address of pool
 */

/**
 * Setup Paycer pool for testing
 *
...
/**
 * Setup Paycer Earn Drip Pool for testing
 *
...
async function setupEarnDrip(growPool = Address.vDAI) {
  beforeEach(async function () {
    ...
    const initData = paycerEarnDripImpl.interface.encodeFunctionData('initialize', [this.pool.address, [growPool]])
    ...
    this.earnDrip = await ethers.getContractAt('PaycerEarnDrip', proxy.address)

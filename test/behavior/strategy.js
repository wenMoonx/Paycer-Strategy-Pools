...
const { shouldBehaveLikeEarnPaycerMakerStrategy } = require('../behavior/earn-paycer-maker-strategy')
...
const { shouldBehaveLikeEarnPaycerStrategy } = require('../behavior/earn-paycer-strategy')
...
function shouldBehaveLikeStrategy(strategyIndex, type, strategyName) {
  ...
  const behaviors = {
    ...
    [StrategyType.EARN_PAYCER_MAKER]: shouldBehaveLikeEarnPaycerMakerStrategy,
    ...
    [StrategyType.EARN_PAYCER]: shouldBehaveLikeEarnPaycerStrategy,
  ...
  describe(`${strategyName} Strategy common behaviour tests`, function () {
    ...
    describe('New strategy migration', function () {
      it('Should revert if caller is not paycer pool', async function () {
        await expect(strategy.migrate(ANY_ERC20)).to.be.revertedWith('caller-is-not-paycer-pool')
      })
    })
    ...
    describe('Only pool can call withdraw', function () {
      it('Should not be able to withdraw from pool', async function () {
        await expect(strategy.withdraw(1)).to.be.revertedWith('caller-is-not-paycer-pool')
      })
    })

...
contract PaycerEarnDrip is PoolRewards {
        ...
        returns (address[] memory _rewardTokens, uint256[] memory _claimableAmounts)
    {
        ...
        for (uint256 i = 0; i < _len; i++) {
            ...
            if (rewardTokens[i] == growToken) {
                _claimableAmount = (IPaycerPool(growToken).pricePerShare() * _claimableAmount) / 1e18;
            }
    ...
    function notifyRewardAmount(
        ...
        (bool isStrategy, , , , , , , ) = IPaycerPool(pool).strategy(msg.sender);
        require(
            msg.sender == IPaycerPool(pool).governor() || (isRewardToken[_rewardToken] && isStrategy),
            "not-authorized"
        );
        super._notifyRewardAmount(_rewardToken, _rewardAmount, _rewardDuration, IPaycerPool(pool).totalSupply());
    }
    ...
    function _transferRewards(
        ...
        if (_rewardToken == growToken) {
            ...
            IERC20 _dripToken = IPaycerPool(_rewardToken).token();
            uint256 _dripBalanceBefore = _dripToken.balanceOf(address(this));
            IPaycerPool(_rewardToken).withdraw(_reward);

...
contract PoolAccountantStorageV1 {
    address public pool; // Address of Paycer pool
...
/// @title Accountant for Paycer pools which keep records of strategies.
contract PoolAccountant is Initializable, PoolAccountantStorageV1, Context {
     ...
     * @param _pool Address of Paycer pool proxy
     */
    ...
    modifier onlyGovernor() {
        require(IPaycerPool(pool).governor() == _msgSender(), "not-the-governor");
        _;
    }

    modifier onlyKeeper() {
        require(IPaycerPool(pool).keepers().contains(_msgSender()), "not-a-keeper");
        _;
    }

    modifier onlyMaintainer() {
        require(IPaycerPool(pool).maintainers().contains(_msgSender()), "not-a-maintainer");
        _;
    }
    ...
    {
        require(strategy[_strategy].active, Errors.STRATEGY_IS_NOT_ACTIVE);
        require(IPaycerPool(pool).token().balanceOf(_strategy) >= (_profit + _payback), Errors.INSUFFICIENT_BALANCE);
        if (_loss != 0) {
    ...
    function _reportLoss(address _strategy, uint256 _loss) internal {
        ...
        uint256 _deltaDebtRatio =
            _min((_loss * MAX_BPS) / IPaycerPool(pool).totalValue(), strategy[_strategy].debtRatio);
    ...
    function _availableCreditLimit(address _strategy) internal view returns (uint256) {
        if (IPaycerPool(pool).stopEverything()) {
            return 0;
        }
        uint256 _totalValue = IPaycerPool(pool).totalValue();
        ...
        _available = _min(_min(IPaycerPool(pool).tokensHere(), _available), _poolDebtLimit - totalDebt);
        _available = _min(
    ...
    function _excessDebt(address _strategy) internal view returns (uint256) {
        ...
        uint256 _maxDebt = (strategy[_strategy].debtRatio * IPaycerPool(pool).totalValue()) / MAX_BPS;

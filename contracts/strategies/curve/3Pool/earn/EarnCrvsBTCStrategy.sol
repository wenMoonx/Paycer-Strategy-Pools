...
contract EarnCrvsBTCStrategy is CrvsBTCPoolStrategy, Earn {
    ...
    function rebalance() external override(Strategy, Crv3PoolStrategyBase) onlyKeeper {
        ...
        IPaycerPool(pool).reportEarning(0, _loss, _payback);
        _reinvest();
        if (!depositError) {
            uint256 depositLoss = _realizeLoss(IPaycerPool(pool).totalDebtOf(address(this)));
            if (depositLoss > _loss) IPaycerPool(pool).reportLoss(depositLoss - _loss);
        }
    }
